...
$token = [CM5]::GetPassword("Supabase CLI:supabase")
$out = "C:\Users\Lenovo\Downloads\JobGuinee\mig5.log"
...
# Test simple query
RunSQL "SELECT 1 as test" "Test"

# Step 1: Add columns
RunSQL "ALTER TABLE profiles ADD COLUMN IF NOT EXISTS is_account_confirmed boolean DEFAULT true" "AddCol1"
RunSQL "ALTER TABLE profiles ADD COLUMN IF NOT EXISTS confirmation_token uuid" "AddCol2"

# Step 2: Update existing users
RunSQL "UPDATE profiles SET is_account_confirmed = true WHERE is_account_confirmed IS NULL" "UpdateExisting"

# Step 3: Create index
RunSQL "CREATE INDEX IF NOT EXISTS idx_profiles_confirmation_token ON profiles(confirmation_token) WHERE confirmation_token IS NOT NULL" "CreateIndex"

# Step 4: handle_new_user function
$fnHandleNewUser = @"
...
VALUES (
  ...
  'Confirmez votre inscription sur JobGuinee',
  '<div style=""font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;"">
    <div style=""text-align: center; margin-bottom: 30px;"">
      <h1 style=""color: #1e3a5f; margin: 0;"">JobGuinee</h1>
      <p style=""color: #666; font-size: 14px;"">La plateforme emploi de la Guinee</p>
    </div>
    <h2 style=""color: #333;"">Bienvenue {{user_name}} !</h2>
    <p style=""color: #555; font-size: 16px; line-height: 1.6;"">
      Vous vous etes inscrit(e) sur <strong>JobGuinee</strong> avec votre compte Google.
    ...
    <p style=""color: #999; font-size: 12px; text-align: center;"">
      Si vous n''avez pas cree de compte sur JobGuinee, ignorez cet email.
    </p>
  </div>',
  true,
  now(),
