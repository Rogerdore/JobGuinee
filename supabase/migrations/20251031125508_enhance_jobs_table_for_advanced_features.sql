/*
  ...
  ## Overview
  This migration adds new columns to the jobs table to support the complete job board features
  including sector filtering, experience levels, view tracking, featured/urgent posts, and more.

  ## Changes to jobs table
  
  ### New Columns Added:
  1. `sector` (text) - Industry sector (Mines, RH, Finance, Logistique, Construction, etc.)
  2. `experience_level` (text) - Required experience (Débutant, 1-3 ans, 3-5 ans, +5 ans)
  3. `education_level` (text) - Required education level
  4. `views_count` (integer) - Number of times the job was viewed
  5. `applications_count` (integer) - Number of applications received
  6. `is_featured` (boolean) - Whether this is a featured/sponsored job
  7. `is_urgent` (boolean) - Whether this is marked as urgent
  8. `requirements` (text) - Detailed requirements and qualifications
  9. `responsibilities` (text) - Job responsibilities
  10. `benefits` (text) - Benefits and perks
  11. `deadline` (timestamptz) - Application deadline
  12. `nationality_required` (text) - Local/Expatriate/Any
  13. `languages` (text[]) - Required languages
  14. `keywords` (text[]) - Search keywords for better matching

  ## Purpose
  - Enable advanced filtering and search capabilities
  - Track job performance metrics
  - Support premium/featured listings
...
-- Create indexes for saved_jobs and job_views
