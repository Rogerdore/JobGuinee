/*
  # Ajout du tracking dans job_clicks

  1. Modifications
    - Modifie la fonction `track_job_view_secure` pour enregistrer les clics dans `job_clicks`
    - Permet de suivre les offres consultées par chaque utilisateur
    - Maintient la compatibilité avec le système existant

  2. Résultat
    - Chaque consultation d'offre sera enregistrée dans `job_clicks`
    - L'indicateur "Offres consultées" affichera le nombre correct
*/
...
BEGIN
  ...
  ELSE
    -- Utilisateur anonyme : hash simple avec md5
  ...
  -- NOUVEAU: Enregistrer le clic dans job_clicks
  ...
  -- Logger le succès dans candidate_stats_logs
  INSERT INTO candidate_stats_logs (
    candidate_id,
    stat_type,
    source,
  ...
  -- Si utilisateur connecté, mettre à jour ses stats (optionnel, car on compte maintenant depuis job_clicks)
  IF v_user_id IS NOT NULL THEN
