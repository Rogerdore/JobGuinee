/*
  # Apply Advanced ATS Workflow System
  
  1. New Tables
    - workflow_stages: Custom recruitment workflow stages per company
    - application_notes: Recruiter notes on applications
    - application_timeline: Complete history of application status changes
    - recruiter_messages: Direct messaging between recruiters and candidates
    - recruitment_analytics: Daily analytics snapshots
  
  2. Updates to Existing Tables
    - applications: Add ai_score, ai_category, workflow_stage, cv_url, recruiter_notes, ai_match_explanation
    - jobs: Add department, ai_generated, hiring_manager_id
  
  3. Security
    - Enable RLS on all tables
    - Policies for recruiters, candidates, and admins
    
  4. Automation
    - Trigger to create default workflow stages for new companies
    - Trigger to log all application changes to timeline
*/
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE applications ADD COLUMN workflow_stage text DEFAULT 'Candidature reçue';
  END IF;
